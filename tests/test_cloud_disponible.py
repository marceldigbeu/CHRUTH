...
_CLES = ("ANTHROPIC_API_KEY", "MISTRAL_API_KEY", "GROQ_API_KEY", "GEMINI_API_KEY")
...
def test_cloud_provider_gemini_seul(monkeypatch):
    _clean(monkeypatch)
    monkeypatch.setenv("GEMINI_API_KEY", "x")
    assert llm_client.cloud_provider() == "gemini"
    assert llm_client.cloud_disponible() is True


def test_cloud_provider_gemini_apres_groq(monkeypatch):
    _clean(monkeypatch)
    monkeypatch.setenv("GEMINI_API_KEY", "x")
